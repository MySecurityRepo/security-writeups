...
For every actions, especially creations and modifications the access control has to be performed on the backend. In our case, for example, since we are working with a session-based authentication, if we want to modify a post, we should check that the authenticated user id matches the id of the post author, if it doesn't we should reject the request. A similar approach should be used for every action which aims to modify something, or for every request which aims to read confidential content.
